...
### 1.1 — `<main>` يستخدم كـ Flex Container لا كـ Landmark ✅ **[تم الإصلاح]**
...
### 1.2 — أزرار بدون `aria-label` في مناطق حرجة ✅ **[تم الإصلاح - المواضع الحرجة]**
...
### 1.3 — `console.log` واحد في production ✅ **[مقصود - ErrorBoundary]**
...
### 1.4 — لا يوجد إدارة لـ `document.title` ✅ **[تم الإصلاح بـ usePageTitle]**
...
### 1.5 — نموذج `AddNewFact.tsx` بدون `dir="rtl"` ✅ **[تم الإصلاح]**
...
### 1.6 — 84 موضع `transition-all` بدون `prefers-reduced-motion` ✅ **[تم الإصلاح في index.css]**
...
### 2.1 — `bg-white` بدون `dark:` في 5 مواضع ✅ **[تم الإصلاح]**
...
### 2.2 — `dark:bg-[#1A1A1A]` مكتوبة بشكل حرفي 21 مرة ✅ **[تم الإصلاح]**
...
### 3.1 — 19 لون hex مكتوب مباشرة في `style={{}}` ✅ **[تم الإصلاح]**
...
### 3.3 — 5 أزرار تستخدم `bg-[var(--main-color)]` بدلاً من `<Button color="primary">` ✅ **[تم الإصلاح]**
...
### 4.3 — حقول التاريخ (`type="date"`) بدون `aria-label` واضح ✅ **[تم الإصلاح]**
...
### 4.4 — 10 نماذج `<form>` بدون `dir="rtl"` ✅ **[تم الإصلاح]**
```
...
### 4.5 — `AddNewCaseFromOCRForm` — خطوة اختيار الطرف (defending party) بدون `role="radiogroup"` ✅ **[تم الإصلاح]**
زرا "الموكل / الخصم" في `step='defending'` يبدوان كـ radio buttons لكن بدون `role="radiogroup"` / `role="radio"` / `aria-checked`.

---

### 4.6 — نموذج الدفع (`PaymentModal`) — حالة اختيار طريقة الدفع تفتقر لتأكيد مرئي ✅ **[تم الإصلاح]**
...
### 5.1 — الصفحات لا تعلن نفسها (لا عنوان مميز) ✅ **[تم الإصلاح]**
...
### 5.2 — زر "رجوع" في `WorkflowStepBar` غير موجود على الموبايل ✅ **[تم الإصلاح]**
...
### 5.3 — `ClarifyFactsModal` بحجم `size="3xl"` على جميع الشاشات ✅ **[تم الإصلاح]**
...
### 5.4 — لا يوجد Breadcrumb في صفحات القضية المتداخلة ✅ **[تم الإصلاح]**
...
### 7.1 — خلط بين `bg-white` و `app-surface` ✅ **[تم الإصلاح]**
`DefensesList.tsx` و `LegalAnalysis.tsx` يستخدمان `bg-white` مباشرة في 12+ موضع، مما يكسر dark mode.

---

### 7.2 — `text-[11px]` لا تزال موجودة في 43 موضع ✅ **[تم الإصلاح]**
...
### 7.3 — `21` حالة `dark:bg-[#1A1A1A]` مكتوبة مباشرة ✅ **[تم الإصلاح]**
بدلاً من `dark:app-surface` — راجع 2.2.

---

### 7.4 — `SkeletonCards.tsx` بارتفاع ثابت `h-[300px]` ✅ **[تم الإصلاح - min-h]**
...
### 7.5 — `SkeletonForm.tsx` بعرض ثابت `w-[25px]` ✅ **[تم الإصلاح]**
...
### 8.2 — `eslint-disable` في `DefensesList.tsx` لـ missing dependency ✅ **[تم الإصلاح]**
...
### 9.1 — "المحادثه الذكيه" — خطأ إملائي ✅ **[تم الإصلاح]**
...
### 9.2 — "الاعدادات" — بدون همزة ✅ **[تم الإصلاح]**
...
### 9.3 — `FinalStatementOfClaims` — نص PDF يستخدم `color:#666` hardcoded ✅ **[تم الإصلاح - موثقة كـ Intentional]**
**الملفات:**
...
### 9.4 — "الصفحة الرئيسة" — بدون تاء التأنيث الصحيحة ✅ **[تم الإصلاح]**
...
3. ~~**30+ زر بدون `aria-label`** → مسح شامل بـ `aria-label`~~ ✅ (للمناطق الحرجة)
...
7. ~~**`console.log` في production** → احذفه~~ ✅ (مقصود في ErrorBoundary)
